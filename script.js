...
const win = (playerArray, color) => {
    for (let index = 0; index < winningArr.length; index++) {
       let count = 0;
         for (let j = 0; j < winningArr[index].length; j++) if (playerArray.includes(winningArr[index][j]))  count++;   

            if (count === 3) {

          setTimeout(() => {    winningArr[index].forEach((val) => { 
...
let winO = win(placeHolderArrO, '#F2B137');
