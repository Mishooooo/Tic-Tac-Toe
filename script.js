...
computer.addEventListener('click', () => {
  ...
  hideLobby.style.display = 'none';
   ...
   document.querySelector(`#x_score_label`).textContent = player1or2 === 1 ? `X (YOU)`: `O (CPU)`;
   document.querySelector(`#o_score_label`).textContent = player1or2 === 1 ?  `O (CPU)` : `O (YOU)`;
...
if (vsComputer && !placeHolderArrO.includes(i) && !placeHolderArrX.includes(i)){
...
placeHolder[i - 1].insertAdjacentHTML('afterbegin', player1or2 == 1 ? xIcon : oIcon);

 if(validRandomNums.length !== 0) setTimeout(() =>  placeHolder[validRandomNums[randomNum] - 1].insertAdjacentHTML("afterbegin", player1or2 == 1 ? oIcon : xIcon), 150);

  placeHolderArrX[i - 1] = player1or2 == 1 ? i : validRandomNums[randomNum];
  placeHolderArrO[i - 1] = player1or2 == 1 ? validRandomNums[randomNum] : i;
  
whoseTurn.src = player1or2 === 1 ? "./assets/icon-o.svg" : "./assets/icon-x.svg"
setTimeout (() =>   whoseTurn.src = player1or2 === 1 ? "./assets/icon-x.svg" : "./assets/icon-o.svg", 150);
}
...
markedX = i;
      activePlayer = 1;
}  else if (activePlayer === 1 && !markedX &&  placeHolderArrO[i - 1] !== i){
placeHolder[i - 1].insertAdjacentHTML('afterbegin', oIcon)
 ...
 whoseTurn.src = activePlayer === 0 ? "./assets/icon-x.svg" : "./assets/icon-o.svg";
...
let tie = placeHolderArrO.concat(placeHolderArrX).filter( int => {
  return typeof(int) === 'number' }).reduce((acc, cur) => {
return acc += cur
}, 0);
...
winningArr.forEach((winningComb) => {


const  win = function (arrVAlues) {
 const filteredArr = arrVAlues.filter( int => {
      return typeof(int) === 'number' 
    });
   return  filteredArr.includes(winningComb[0]) && filteredArr.includes(winningComb[1]) && filteredArr.includes(winningComb[2]);
...
const notPushInArr = (el) => el.splice(el.findIndex( ind => ind ===  validRandomNums[randomNum]), 1);
if(vsPlayer) {
 ...
 const placeHolderColor = function (color, n){
  setTimeout(() => { 
  placeHolder[winningComb[n] - 1].classList.add('marked_xo');
  placeHolder[winningComb[n] - 1].style.backgroundColor = color;
 }, 150)
  };


let winO = win(placeHolderArrO);
let winX = win(placeHolderArrX);
...
if (vsComputer && validRandomNums.length !== 0 && (player1or2  == 1 ? winX : winO) )  setTimeout(() => placeHolder[validRandomNums[randomNum] - 1].innerHTML = '', 150);


setTimeout(() => showWhoWins(whoWins), 1200); 


 placeHolderArrX.filter( int => typeof(int) === 'number');


for (let index = 0; index <= 2; index++) placeHolderColor(winX ? '#31C3BD' : '#F2B137', index);


whoWonGame( `<div><h2>${oneOrTwo} </h2></div>
  <div>  <h1><img src=${winX ? './assets/icon-x.svg' : './assets/icon-o.svg'} alt="XO icon"> TAKES THE ROUND</h1></div>`);
  


winX ? scoreXIndicator++ : scoreOIndicator++; 
if (winX || winO)document.querySelector(`.XorOscored${winX ? 1 : 0 }`).textContent = winX ? scoreXIndicator : scoreOIndicator;
...
xoOutline = activePlayer === 0 ?'x': 'o';
  ...
  if(!placeHolderArrO.includes(i) && !placeHolderArrX.includes(i) ) return placeHolder[i -1].insertAdjacentHTML("afterbegin",  `<img src="./assets/icon-${xoOutline}-outline.svg" alt="O Icon">`);
}

const clearPlaceholder = function(){
  if(!placeHolderArrO.includes(i) && !placeHolderArrX.includes(i)) return placeHolder[i - 1].innerHTML = '';
}
